...
def rs_padding(img, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32):
    ...
    if shape[::-1] != new_unpad:  # resize
        pad_img = cv2.resize(img, new_unpad, interpolation=cv2.INTER_LINEAR)
    
    pad_img = cv2.copyMakeBorder(img, 0, dh, 0, 0, cv2.BORDER_CONSTANT, value=color)  # add border
    return pad_img, ratio, (dw, dh)
